...
from atlazer.celery_app.tasks.workspace.material import build_sources, save_material, upload_material
...
async def gemini_batch_webhook(request: Request):
    ...
    if data and user_metadata and batch_type == "batch.succeeded":
        ...
        if action == 'context_papers_summary_generation':
            ...
            job = save_summaries.apply_async(
                kwargs={"metadata": metadata},
                queue="workspace"
            )
        ...
        if action == 'material_builder':
            log.info(
                "webapi.gemini-batch-webhook.save_material.start",
                user_metadata=user_metadata
            )
            ...
            job = (
                save_material.s(metadata).set(queue="workspace")
                | signature(
                    "atlazer.celery_app.tasks.workspace.material.upload_material",
                    queue="workspace",
                    immutable=False,
                )
            ).apply_async()
                
            log.info("webapi.gemini-batch-webhook.save_material.success", task_id=job.id)
            return TaskExecutionResponse(task_id=job.id)
...
def embed_context(payload: EmbedContextRequest):
    ...
    try:
        ...
        job = (
            ...
            | signature(
                "atlazer.celery_app.tasks.workspace.context.summarize_similarities",
                queue="workspace",
                immutable=False,
            )
        ).apply_async()
