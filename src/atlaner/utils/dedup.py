...
Two Redis sets are used:
  atlaner_rag:{repository}:queued    – paper is in flight (downloaded / being processed)
  atlaner_rag:{repository}:processed – paper has been fully stored in the vector DB
...
def is_already_processed(paper_id: str, repository: str) -> bool:
    ...
    r = _get_redis()
    return (
        cast(bool, r.sismember(f"atlaner_rag:{repository}:processed", paper_id))
        or cast(bool, r.sismember(f"atlaner_rag:{repository}:queued", paper_id))
    )
...
def mark_as_queued(paper_id: str, repository: str) -> None:
    ...
    r = _get_redis()
    pipe = r.pipeline()
    pipe.sadd(f"atlaner_rag:{repository}:queued", paper_id)
    pipe.setex(f"atlaner_rag:{repository}:queued:{paper_id}", _QUEUED_TTL_SECONDS, "1")
    pipe.execute()
...
def mark_as_processed(paper_id: str, repository: str) -> None:
    ...
    r = _get_redis()
    pipe = r.pipeline()
    pipe.srem(f"atlaner_rag:{repository}:queued", paper_id)
    pipe.delete(f"atlaner_rag:{repository}:queued:{paper_id}")
    pipe.sadd(f"atlaner_rag:{repository}:processed", paper_id)
    pipe.execute()
...
def reset_paper(paper_id: str, repository: str) -> None:
    ...
    r = _get_redis()
    pipe = r.pipeline()
    pipe.srem(f"atlaner_rag:{repository}:processed", paper_id)
    pipe.srem(f"atlaner_rag:{repository}:queued", paper_id)
    pipe.delete(f"atlaner_rag:{repository}:queued:{paper_id}")
    pipe.execute()
...
def count_processed(repository: str) -> int:
    return cast(int, _get_redis().scard(f"atlaner_rag:{repository}:processed"))


def count_queued(repository: str) -> int:
    return cast(int, _get_redis().scard(f"atlaner_rag:{repository}:queued"))
